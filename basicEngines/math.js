...
{//classes
	vec2_mat2:{
		...
		Math.Vector2=class extends Array{
			...
			//[0]=0;[1]=0;
			scale(float){return Math.scaleVec2(this,float);}
			mapSet(callBack){return this.set(this.map(callBack));}
			length(){return Math.hypot(this[0],this[1]);}
			dot(vec){return this[0]*vec[0]+this[1]*vec[1];}
		};
		Math.mat2=function(){
		...
		Math.Matrix2x2=class extends Array{//new mat2x2([[xx,xy],[yx,yy]]);
			constructor(array2D=[[1,0],[0,1]]){
				if(typeof array2D =="number")super(array2D);
				else super(...array2D);
				/*super([1,0,0,1]);
	...
	mat:{
		...
		const matClass=class Matrix extends Array{
			...
			round(dp=3){return new Matrix(this.map(v=>v.map(v=>+v.toFixed(dp))))}
			aGmat(){return Math.aGmat(this);}
			Gmat(...vecArgs){return this.mul(Math.Gmat(...vecArgs));}
	...
	}
	{
		...
		Math.aGmat=function aGmat(mat,curve=Math.Gmat.curve){
			let ans=[];
